...
*/
  while (running) {
    ...
    */
    ...
    if (medA.getDistance() < 30 && medB.getDistance() > 40) {
      turning = 0;
      if (straight == 0) {
    ...
    } else if ((medA.getDistance() < 30 && medB.getDistance() < 40) || (medA.getDistance() > 30)) {
      straight = 0;
      if (turning == 0) {
        curAngle = gyro.get_angle();
      }
      turning++;
      drive.drive(curAngle + 10, gyro.get_angle(), .25); //turn away from wall
      straight = 0;
      sleep(.5);
