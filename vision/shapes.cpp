...
//       printf("--------------------------------\nNumber of vectors in contour and aprox #%d: %d, %d \n---------------------------------",i,contours[i].size(),approxit.size());
	...
	hexagons.push_back(approxit);drawContours( drawing, hexagons, cuenta, color, 2),cuenta++;
        Moments mu = moments( approxit, false ); Point2f mc = Point2f( mu.m10/mu.m00 , mu.m01/mu.m00 );
        circle(drawing, mc, 3,color, 1);
}
     }
